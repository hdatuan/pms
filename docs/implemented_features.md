...
* **Thêm người dùng**: Hoàn thiện qua [UserController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/UserController.java) (`/user-add`).
* **Chỉnh sửa thông tin**: Hoàn thiện qua [UserController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/UserController.java) (`/user-edit`). *(Trước đây tài liệu cũ bỏ sót tính năng này).*
...
### 5. Quản lý Tác vụ (Task Management) - *Đầy đủ CRUD*
* **Xem danh sách**: Hoàn thiện qua [TaskController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/TaskController.java) (`/task`). Hiển thị badge màu theo trạng thái. Phân quyền: chỉ Admin/Manager thấy nút hành động.
* **Thêm tác vụ**: Hoàn thiện qua [TaskController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/TaskController.java) (`/task-add`). Form chọn dự án, người thực hiện, trạng thái từ dropdown động. Chỉ Admin/Manager.
* **Chỉnh sửa tác vụ**: Hoàn thiện qua [TaskController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/TaskController.java) (`/task-edit`). Form pre-filled, dropdown selected đúng giá trị hiện tại. Chỉ Admin/Manager.
* **Xóa tác vụ**: Hoàn thiện qua [TaskController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/TaskController.java) (`/task-delete`). Confirm dialog trước khi xóa, redirect về danh sách với flash message. Chỉ Admin/Manager.
* **Thông báo**: Tích hợp `showToast` nhất quán với User, Job, Role — thành công redirect về danh sách, thất bại ở lại trang.
...
* **Cập nhật tiến độ**: Hoàn thiện qua [ProfileController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/ProfileController.java) (`/profile-edit`), cho phép nhân viên cập nhật ngày bắt đầu, ngày kết thúc và trạng thái của tác vụ. *(Lỗ hổng bảo mật phân quyền ngang IDOR đã được khắc phục triệt để bằng cách đối chiếu thông tin user).*
...
* **Mô tả**: Tại lớp [ProfileController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/ProfileController.java) (trước đây là `ProfileEditController`) (phương thức `doPost`), hệ thống cập nhật tác vụ dựa vào tham số `taskId` lấy trực tiếp từ request mà **không kiểm tra xem tác vụ đó có thực sự thuộc về người dùng đang đăng nhập hay không**.
...
* **Mô tả**: Các thao tác cho cùng một thực thể (ví dụ: `Role`, `User`, `Profile`) đang được chia nhỏ ra rất nhiều Servlet độc lập (`RoleAddController`, `RoleDeleteController`, `UserAddController`, `ProfileEditController`). Điều này gây khó khăn cho việc quản lý mã nguồn và bảo trì cấu hình URL.
* **Khắc phục**: Đã gộp toàn bộ các Servlet quản lý vai trò vào [RoleController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/RoleController.java), quản lý người dùng vào [UserController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/UserController.java), và quản lý cá nhân vào [ProfileController](file:///d:/WorkSpace/pms/src/main/java/hdatuan/controller/ProfileController.java) để điều hướng hành động thông qua `ServletPath` (như `/role`, `/role-add`, `/user-add`, `/user-edit`, `/profile-edit`). Các file Controller cũ phân mảnh đã được xóa bỏ để giữ cấu trúc dự án tinh gọn.
